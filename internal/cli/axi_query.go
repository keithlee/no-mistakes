...
func newAxiLogsCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&step, "step", "", "step name: intent, rebase, review, test, proof, proof-review, document, lint, push, pr, ci (required)")
...
func runAxiLogs(cmd *cobra.Command, step, runID string, full bool) (string, error) {
	...
	if step == "" {
		return "", emitError(cmd, 2, "--step is required",
			"Valid steps: intent, rebase, review, test, proof, proof-review, document, lint, push, pr, ci")
	}
