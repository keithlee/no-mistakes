...
- `agent_path_override`, `agent_config`, `agent_args_override`, `acpx_path`, `acp_registry_overrides`, `ci_timeout`, `daemon_connect_timeout`, `branch_sync_remote_timeout`, `step_quiet_warning`, `agent_timeout`, `review_agent_timeout`, `test_agent_timeout`, `log_level`, `session_reuse`, and `proof.guidance_files` are global-only fields.
...
## Operator proof guidance

Set `proof.guidance_files` in the global config to one or more absolute paths
owned by the operator. Before each Proof gate, no-mistakes snapshots and hashes
the files; missing, unreadable, invalid, duplicated, or oversized files block
the run. Repository configuration cannot add, replace, or disable these files.
ProofReview is a fresh independent acceptance review, so its pass is tied to
the exact head and its proof artifacts.
