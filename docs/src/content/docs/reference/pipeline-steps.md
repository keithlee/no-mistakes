...
- In this fork, `checks-passed` is also gated by a fresh current-head review that follows the installed `rigorous-output-proof` skill. It independently inspects the diff, actual outputs, and Test evidence; missing, stale, unreadable, or contradictory proof parks for a human decision instead of handing off
- On GitHub, reads every paginated review submission, review thread and nested reply, and PR conversation comment on each poll. Human feedback and bot feedback are in scope except the PR author's own comments, resolved threads, empty review submissions, and no-mistakes disposition markers
- Classifies each new comment as an objective correction, an acknowledgement that needs no code change, or a product/scope decision. Objective corrections use the existing CI repair path and restart at Review. Acknowledgements wait for current-head validation. Both receive a factual head-bound reply only after current-head proof and CI pass; inline threads are then resolved. Ambiguous feedback parks for the human. The restart-safe reconciliation ledger prevents a repair from being acknowledged before validation
...
- On GitHub, also includes unresolved review-thread comments from supported review bots (currently Greptile) in ordinary CI-failure repair prompts; the comments are framed as untrusted external data and the rendered section is capped at 32 KiB
...
- If a fix attempt produces no changes: a trusted conclusion that the failure is not caused by the PR's code stops immediately and reports the agent's summary. Otherwise, automatic mode leaves the failure undeduplicated so a genuine code defect can retry until the auto-fix limit, while manual fix mode returns immediately for manual intervention
