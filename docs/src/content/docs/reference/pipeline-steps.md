...
Items are ordered by the fixed pipeline order and represent the exact database snapshot when no-mistakes creates or updates the PR body. The attestation includes `pr` and `ci` records even though their human-readable details are not shown in `## Pipeline`; at the normal PR write point those records are commonly `running` and `pending`. The `head_sha` binds that snapshot to the commit it describes, so consumers can detect when a later push has made the comment stale. After a pipeline repair push, no-mistakes rebinds an existing attestation to the newly published head so a PR that was raised through the gate stays attested; a PR that never carried an attestation is left unchanged.
