...
- **Before** the code is public, it rebases, runs a structured AI code review, runs baseline tests, produces and independently reviews proof evidence, checks that docs are in sync, runs lint, and only then pushes to the configured target and opens the PR.
...
- review, tests, proof production, independent proof review, docs, and lint happened before the branch reached the configured push target
- you had a chance to approve, fix, skip, or abort any blocking step

## What you get

- A fixed, opinionated pipeline: `intent → rebase → review → test → proof → proof-review → document → lint → push → pr → ci`. Order is not configurable; what each step runs is.
